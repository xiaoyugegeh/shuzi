    ...
    <div className="flex h-full flex-col">
      ...
      <div className="flex items-center justify-between border-b border-zinc-800/60 px-4 py-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium text-zinc-200">Agent 问答</span>
        </div>
        {messages.length > 0 && (
              ...
              {['RAG 技术的核心流程是什么？', 'React 18 有哪些新特性？', '如何选择向量数据库？'].map(
                (suggestion) => (
                  <button
                    ...
                    className="block w-full rounded-lg border border-zinc-800/60 bg-zinc-900/30 px-4 py-2 text-left text-xs text-zinc-500 transition-all hover:border-zinc-700 hover:bg-zinc-800/50 hover:text-zinc-300"
                  >
                    {suggestion}
                  </button>
                )
          ...
          <div className="space-y-4">
            ...
            {isProcessing && (
              <div className="flex gap-3">
                <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-zinc-800 text-zinc-400 ring-1 ring-zinc-700/50">
                  <Sparkles size={14} className="animate-pulse" />
                </div>
                <div className="rounded-xl bg-zinc-800/60 px-4 py-2.5 ring-1 ring-zinc-700/30">
